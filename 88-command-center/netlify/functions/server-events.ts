...
export const handler: Handler = async (event) => {
  try {
    ...
    try {
      upstream = await fetch(`${botApiUrl}/api/server/${serverId}/events`, {
        headers: { "X-Api-Key": botApiKey, "X-Discord-User-Id": claims.sub },
      });
    } catch (err) {
      console.error("[server-events fn] fetch failed:", err);
      return {
        statusCode: 502,
    ...
    if (!upstream.ok) {
      console.error("[server-events fn] upstream error", { status: upstream.status, bodyPreview: text.slice(0, 500) });
    }
    return {
    ...
    console.error("[server-events fn] error:", msg);
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json", "Cache-Control": "no-store" },
