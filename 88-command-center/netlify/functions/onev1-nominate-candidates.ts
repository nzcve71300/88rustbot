...
export const handler: Handler = async (event) => {
  try {
    ...
    try {
      upstream = await fetch(`${botApiUrl}/api/server/${serverId}/onev1/nominate-candidates`, {
        headers: { "X-Api-Key": botApiKey, "X-Discord-User-Id": claims.sub },
      });
    } catch (err) {
      console.error("[onev1-nominate-candidates fn] fetch failed:", err);
      return {
        statusCode: 502,
    ...
    console.error("[onev1-nominate-candidates fn] error:", msg);
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json", "Cache-Control": "no-store" },
