...
export const handler: Handler = async (event) => {
  try {
    ...
    let rawBody = "{}";
    try {
      rawBody = event.body && event.isBase64Encoded ? Buffer.from(event.body, "base64").toString("utf8") : event.body ?? "{}";
    } catch {
      rawBody = "{}";
    }
    ...
    try {
      upstream = await fetch(`${botApiUrl}/api/server/${serverId}/onev1/nominate`, {
        method: "POST",
        headers: {
          "X-Api-Key": botApiKey,
      ...
      console.error("[onev1-nominate fn] fetch failed:", err);
      return {
        statusCode: 502,
    ...
    console.error("[onev1-nominate fn] error:", msg);
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json", "Cache-Control": "no-store" },
