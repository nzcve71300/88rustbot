...
export const handler: Handler = async (event) => {
  try {
    ...
    try {
      upstream = await fetch(`${botApiUrl}/api/server/${serverId}/me/stats`, {
        headers: { "X-Api-Key": botApiKey, "X-Discord-User-Id": claims.sub },
      });
    } catch (err) {
      console.error("[my-stats fn] fetch failed:", err);
      return {
        statusCode: 502,
    ...
    if (!upstream.ok) {
      console.error("[my-stats fn] upstream error", { status: upstream.status, bodyPreview: text.slice(0, 500) });
    }
    return {
    ...
    console.error("[my-stats fn] error:", msg);
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json", "Cache-Control": "no-store" },
