...
export const handler: Handler = async (event) => {
  try {
    ...
    const body = event.body?.trim() ? JSON.parse(event.body) : {};
    const ingameName = String((body as { ingameName?: string }).ingameName ?? "").trim();
    if (!ingameName) return { statusCode: 400, body: JSON.stringify({ ok: false, error: "Missing ingameName" }) };

    const botApiUrl = requireEnv("BOT_API_URL").replace(/\/+$/, "");
    const botApiKey = requireEnv("BOT_API_KEY");
    const upstream = await fetch(`${botApiUrl}/api/server/${serverId}/me/link`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-Api-Key": botApiKey, "X-Discord-User-Id": claims.sub },
