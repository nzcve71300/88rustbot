...
export const handler: Handler = async (event) => {
  try {
    ...
    const eventType = String(event.queryStringParameters?.eventType ?? "").trim().toLowerCase();
    ...
    if (!["koth", "maze", "nuketown"].includes(eventType)) {
      return { statusCode: 400, body: JSON.stringify({ ok: false, error: "Invalid eventType" }) };
    }
    ...
    try {
      upstream = await fetch(`${botApiUrl}/api/server/${serverId}/events/${eventType}/${action}`, {
        method: "POST",
        headers: { "X-Api-Key": botApiKey, "X-Discord-User-Id": claims.sub, "Content-Type": "application/json" },
        body: "{}",
      });
    } catch (err) {
      console.error("[event-action fn] fetch failed:", err);
      return {
        statusCode: 502,
    ...
    if (!upstream.ok) {
      console.error("[event-action fn] upstream error", { status: upstream.status, bodyPreview: text.slice(0, 500) });
    }
    return {
    ...
    console.error("[event-action fn] error:", msg);
    return {
      statusCode: 500,
      headers: { "Content-Type": "application/json", "Cache-Control": "no-store" },
