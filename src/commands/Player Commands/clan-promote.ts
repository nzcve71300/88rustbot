...
import { getMemberClan, promoteClanOwner } from "../../db/clans.js";
...
export const clanPromoteCommand = {
  data: new SlashCommandBuilder()
    .setName("clan-promote")
    .setDescription("Promote a clan member to owner (owner only).")
    .addStringOption((o) =>
      o
        .setName("server")
    ...
    .addUserOption((o) => o.setName("user").setDescription("Member to promote to owner.").setRequired(true)),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!myClan.ownerDiscordUserId || String(myClan.ownerDiscordUserId) !== interaction.user.id) {
      await interaction.editReply({
        embeds: [baseEmbed().setTitle("Not allowed").setDescription("Only the clan owner can promote members.")],
      });
      return;
    }
    ...
    if (!targetClan || targetClan.clanId !== myClan.clanId) {
      await interaction.editReply({
        embeds: [baseEmbed().setTitle("Not in clan").setDescription("That user must be in your clan first.")],
      });
      return;
    }

    await promoteClanOwner(pool, guildRowId, myClan.clanId, target.id);

    await interaction.editReply({
      embeds: [
        baseEmbed()
          .setTitle("New owner promoted")
          .setDescription(`**${target.username}** is now the owner of **${myClan.clanName}**.`),
      ],
    });
  },
};
